            ...
            <div class="card date textDirection table-card d-none">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="deduction_report" class="table table-bordered table-striped text-center d-none date" @if ($currentLang == 'ar') style="direction: rtl; text-align: right" @endif>
                            <thead>
                                <tr>
                                    <th>
                                        @lang('site.serial')
...
{{--                const url = "{{ route('getClientsFromclientType') }}";--}}
    ...
    <script>
        ...
        function submit() {
            ...
            let ajaxURL = `${subFolderURL}/${urlLang}/reports/deduction-report`;
{{--            let ajaxURL = "{{ route('reports.deduction_report_view') }}";--}}
            let ajaxMethod = 'post';
            ...
            $.ajax({
                ...
                success: function(deductions) {
                    ...
                    deductions.forEach((deduction, index) => {
                        ...
                        newRow.append($(`<td>${deduction.total_document_with_tax.toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20})}</td>`));
                        newRow.append($(`<td>${deduction.currency == 'EGP' ?deduction.deduction_value.toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20}) : '_'}</td>`));
                    ...
                    $('#total-number').text(`( ${summationMap.get('EGP').toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 7})} )`);
                    ...
                    summationMap.forEach(function(value, key) {
                        ...
                        newCurrencyRow.append($(`<td>${value.toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 7})}</td>`));
