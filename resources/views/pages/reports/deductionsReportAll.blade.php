...
{{--                const url = "{{ route('getClientsFromclientType') }}";--}}
    ...
    <script>
        ...
        function submit() {
            ...
            let ajaxURL = `${subFolderURL}/${urlLang}/reports/deductions-report-all`;
            let ajaxMethod = 'post';
            ...
            $.ajax({
                ...
                success: function (deductions) {
                    ...
                    deductions.forEach((deduction, index) => {
                        ...
                        newRow.append($(`<td>${deduction.name}</td>`));
                        newRow.append($(`<td>${deduction.deductionType}</td>`));
                        newRow.append($(`<td>EGP</td>`));
                        newRow.append($(`<td>${deduction.deduction_value_EGP.toLocaleString('us', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 7
                    })}</td>`));
                        newRow.append($(`<td>USD</td>`));
                        newRow.append($(`<td>${deduction.deduction_value_USD.toLocaleString('us', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 7
                    })}</td>`));                        // newRow.append($(`<td>${deduction.client_name}</td>`));
                    

                        $("#deduction_report tbody").append(newRow);
                    });
                    // $('#total-number').text(`( ${deduction.deduction_value_EGP.toLocaleString('us', {
                    //     minimumFractionDigits: 2,
                    //     maximumFractionDigits: 7
                    // })} )`);
