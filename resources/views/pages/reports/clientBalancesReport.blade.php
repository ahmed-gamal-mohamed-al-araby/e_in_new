...
@endphp
@section('title')
    @lang('site.client_balances_report')
@endsection
...
@section('content')
    ...
    <div class="card">
        <h5 class="card-header bg-success text-center">
            @lang('site.client_balances_report') <span id="total-number"></span>
        </h5>
        <div class="card-body">
            {{-- Date section --}}
            <div class="row date">
            ...
            <div class="card date textDirection table-card d-none">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="clientBalances_report" class="table table-bordered table-striped text-center d-none date" @if ($currentLang == 'ar') style="direction: rtl; text-align: right" @endif>
                            <thead>
                                <tr>
                                    <th>
                                        @lang('site.serial')
                                    ...
                                    <th class="hiddenCols">
                                        @lang('site.total_payemnts')
                                    </th>
                                    <th>
                                        @lang('site.client_balances_at_the_end_of_the_period')
                                    </th>
                                    <th>
                                        @lang('site.currency')
                                    </th>
            ...
            <div class="card date textDirection table-card d-none">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="currencyTable" class="table table-bordered table-striped text-center date" @if ($currentLang == 'ar') style="direction: rtl; text-align: right" @endif>
                            <thead>
                                <tr>
                                    <th>
                                        @lang('site.currency')
...
@section('scripts')
    ...
    <script>
        ...
        // reset table
        function resetTable() {
            $('#clientBalances_report').DataTable().destroy();

            $("#clientBalances_report tbody").html('');
            $('#clientBalances_report').removeClass('d-none');
        }
        ...
        function runDataTable(){
            ...
            $("#clientBalances_report").DataTable({
                "responsive": true,
                "lengthChange": true,
                "autoWidth": false,
                "lengthMenu": [
                ...
                "buttons": [
                    ...
                    {
                        extend: 'excelHtml5',
                        title:  `@lang('site.client_balances_report') @lang('site.to') ( ${dateEnd} ) @lang('site.value'): ${$('#total-number').text()}`,
                        exportOptions: {
                    ...
                    },
                    {
                        ...
                        customize: function(win) {
                            ...
                            win.document.body.getElementsByTagName('h1')[0].innerHTML =
                                "<h3 class='mt-1 mb-3 text-direction'>" + `@lang('site.client_balances_report') @lang('site.to') ( ${dateEnd} ) @lang('site.value'): ${$('#total-number').text()}` + "</h3>",
                ...
                //             // ***
                //             $('#clientBalances_report').find('tr').each(function (ix, row) {
                //                 var index = ix;
                //                 var rowElt = row;
            ...
            }).buttons().container().appendTo('#clientBalances_report_wrapper .col-md-6:eq(0)');

            $('[name="clientBalances_report_length"] option[value="-1"]').attr('selected', true).parent().trigger('change');
    ...
    <script>
        ...
        function submit() {
            ...
            let ajaxURL = `${subFolderURL}/${urlLang}/reports/client-balances`;
            let ajaxMethod = 'post';
            ...
            $.ajax({
                ...
                success: function(clients) {
                    ...
                    clients.forEach((client, index) => {
                        if((client.documentTotalwithTaxes - client.payments) >= 0.1 || (client.documentTotalwithTaxes - client.payments) <= -0.1) { // Avoid display client with total is almost equal to zero
                            ...
                            newRow.append($(`<td>${client.documentTotalwithTaxes.toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 })}</td>`));


                            newRow.append($(`<td>${client.payments.toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 })}</td>`));
                            newRow.append($(`<td>${client.currency == 'EGP' ?(+(Number(client.documentTotalwithTaxes - client.payments).toFixed(8))).toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 }) : '_'}</td>`));
                            // newRow.append($(`<td>${(+(Number(client.documentTotalwithTaxes - client.payments).toFixed(8))).toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 })}</td>`));
                            newRow.append($(`<td>${client.currency}</td>`));
                            // newRow.append($(`<td>${client.currency != 'EGP' ?(+(Number(client.documentTotalwithoutTaxes - client.payments).toFixed(8))).toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 }) : '_'}</td>`));
                            newRow.append($(`<td>${client.currency != 'EGP' ?(+(Number(client.documentTotalwithTaxes - client.payments).toFixed(8))).toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 }) : '_'}</td>`));
                            // newRow.append($(`<td>${(+(Number(client.documentTotalwithTaxes - client.payments).toFixed(8))).toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 })}</td>`));
                            ...
                            if(client.currency == 'EGP') { // Add after taxes
                                summationMap.set(client.currency,(+(Number(summationMap.get(client.currency) + (client.documentTotalwithTaxes - client.payments))))); // update summation
                            } else { // Add without taxes as with taxes multiply item.quantity * item.price * item.rate
                                summationMap.set(client.currency,(+(Number(summationMap.get(client.currency) + (client.documentTotalwithTaxes - client.payments))))); // update summation
                            }

                            $("#clientBalances_report tbody").append(newRow);
                        }
                    });
                    $('#total-number').text(`( ${summationMap.get('EGP').toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 })} )`);
                    ...
                    summationMap.forEach(function(value, key) {
                        ...
                        newCurrencyRow.append($(`<td>${value.toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 })}</td>`));
