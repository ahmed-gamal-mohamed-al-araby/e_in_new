...
@section('content')
    ...
    <div class="card">
        <h5 class="card-header bg-success text-center">
            @lang('site.collections_report') <span id="total-number"></span>
        </h5>
        <div class="card-body">
            {{-- Date section --}}
            <div class="row date">
                ...
                <div class="col-md-3 col-12 textDirection">
                    <div class="input-group mb-3">
                        ...
                        <select id='client_type' name="client_type" class="form-control require" style="width: 100%">
...
@section('scripts')
    ...
    <script>
        ...
        // reset table
        function resetTable() {
            $('#collections_report').DataTable().destroy();

            $("#collections_report tbody").html('');
            $('#collections_report').removeClass('d-none');
        }
        ...
        function runDataTable() {
            ...
            $("#collections_report").DataTable({
                ...
                "buttons": [
                    ...
                    {
                        extend: 'excelHtml5',
                        title: `@lang('site.collections_report') @lang('site.to') ( ${dateEnd} ) @lang('site.value'): ${$('#total-number').text()}`,
                        exportOptions: {
                    ...
                    },
                    {
                        ...
                        customize: function (win) {
                            ...
                            win.document.body.getElementsByTagName('h1')[0].innerHTML =
                                "<h3 class='mt-1 mb-3 text-direction'>" + `@lang('site.collections_report') @lang('site.to') ( ${dateEnd} ) @lang('site.value'): ${$('#total-number').text()}` + "</h3>",
                    ...
                    //             // ***
                    //             $('#collections_report').find('tr').each(function (ix, row) {
                    //                 var index = ix;
                    //                 var rowElt = row;
            ...
            }).buttons().container().appendTo('#collections_report_wrapper .col-md-6:eq(0)');

            $('[name="collections_report_length"] option[value="-1"]').attr('selected', true).parent().trigger('change');
    ...
    <script>
        ...
        function submit() {
            ...
            // let ajaxURL = `${subFolderURL}/${urlLang}/reports/collections`;
            let ajaxURL = "{{ route('reports.collections_get_data') }}";
            let ajaxMethod = 'post';
            ...
            $.ajax({
                ...
                success: function (clients) {
                    ...
                    clients.forEach((client, index) => {
                        ...
                        newRow.append($(`<td>${client.reff}</td>`));
                        newRow.append($(`<td>${client.client_name}</td>`));
                        newRow.append($(`<td>${client.payment}</td>`));


                        newRow.append($(`<td>${client.taxId_NId_VId}</td>`));
                        newRow.append($(`<td>${client.value.toLocaleString('us', {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 20
                        })}</td>`));
                        newRow.append($(`<td>${client.date}</td>`));

                        sum += client.value;
                        $("#collections_report tbody").append(newRow);
                    });
