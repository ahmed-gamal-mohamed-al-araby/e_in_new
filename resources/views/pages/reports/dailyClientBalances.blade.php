...
@endphp
@section('title')
    @lang('site.daily_client_balances_report')
@endsection
...
@section('content')
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active"> <a
                            href="{{ route('payment.index') }}">@lang('site.payment') </a> </li>
                        <li class="breadcrumb-item active">
                            @lang('site.reports')
                        </li>
    ...
    <div class="card">
        <h5 class="card-header bg-success text-center">
            @lang('site.daily_client_balances_report') <span id="total-number"></span>
        </h5>
        <div class="card-body">
            {{-- Date section --}}
            <div class="row date">
            ...
            <div class="card date textDirection table-card d-none">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="dailyClientBalances_report" class="table table-bordered table-striped text-center date" @if ($currentLang == 'ar') style="direction: rtl; text-align: right" @endif>
                            <thead>
                                <tr>
                                    <th>
                                        @lang('site.serial')
...
@section('scripts')
        ...
        function runDataTable(){
            ...
            $("#dailyClientBalances_report").DataTable({
                ...
                "buttons": [
                    ...
                    {
                        extend: 'excelHtml5',
                        title:  `@lang('site.daily_client_balances_report') @lang('site.to') ( ${dateEnd.getDate() + "/" + (dateEnd.getMonth() + 1) + "/" + dateEnd.getFullYear()} ) @lang('site.value'): ${$('#total-number').text()}`,
                        exportOptions: {
                    ...
                    },
                    {
                        ...
                        customize: function(win) {
                            ...
                            win.document.body.getElementsByTagName('h1')[0].innerHTML =
                                "<h3 class='mt-1 mb-3 text-direction'>" + `@lang('site.daily_client_balances_report') @lang('site.to') ( ${dateEnd.getDate() + "/" + (dateEnd.getMonth() + 1) + "/" + dateEnd.getFullYear() } ) @lang('site.value'): ${$('#total-number').text()}` + "</h3>",
            ...
            $('[name="dailyClientBalances_report_length"] option[value="-1"]').attr('selected', true).parent().trigger('change');
        };
    </script>
    ...
    <script>
        ...
        function submit() {
            ...
            let ajaxURL = `${subFolderURL}/${urlLang}/reports/daily-client-balances`;
            let ajaxMethod = 'post';
            ...
            $.ajax({
                ...
                success: function(client) {
                    ...
                    openingBalanceDateMessage = `${openingBalanceDateMessage.slice(0, openingBalanceDateMessage.length-1) + openingBalanceDate.toJSON().slice(0,10).split("-").reverse().join("-")})`;
                   
                    const openingBalanceRow = $('<tr style="background-color: rgba(0,0,0,0.25);"></tr>');
                        openingBalanceRow.append($(`<td>0</td>`));
                        openingBalanceRow.append($(`<td>${client.opening_balance.toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 })}</td>`));
                    ...
                    client.documentsAndPayments.forEach((documentOrPayment, index) => {  // Avoid display client with total is almost equal to zero
                        // Update balance
                        documentOrPayment.P_or_D == "doc"? balance= fixedTo5(balance + documentOrPayment.value) : balance= fixedTo5(balance - documentOrPayment.value);

                        const newRow = $('<tr></tr>');
                        newRow.append($(`<td>${index+1}</td>`));
                        newRow.append($(`<td>${balance.toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 })}</td>`));
                        newRow.append($(`<td>${documentOrPayment.P_or_D == "doc"? '_' : documentOrPayment.value.toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 })}</td>`));
                        newRow.append($(`<td> ${documentOrPayment.P_or_D != "doc"? '_' : documentOrPayment.value.toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 })}</td>`));
                        newRow.append($(`<td> ${documentOrPayment.reff}</td>`));
                        newRow.append($(`<td> ${documentOrPayment.reference}</td>`));
                        
                        @if ($currentLang == 'en')
                            newRow.append($(`<td>${documentOrPayment.date.split("-").reverse().join("-")}</td>`));
                        @else
                    ...
                    $('#total-number').text(`( ${balance.toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 })} )`);
