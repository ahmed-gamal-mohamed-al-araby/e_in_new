        ...
        <div class="row dataDirection">
            ...
            <div class="col-md-4 mb-3">
                <input id="get-recent" type="submit"
                        class="btn btn-success" value="@lang('site.get_recent_documents') (@lang('site.from_eta'))">
            </div>
    ...
    <script>
        ...
        $('#get-recent').on('click', function(e) {
            ...
            var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
                    ...
                    $.ajax({
                        type: 'GET',
                        url: "{{ route('documents.getRecentDocumentsFromServer') }}",
                        headers: {
        ...
        function getDocuments(response) {
            ...
            $('#all-documents').on('click', function (e) {
                e.preventDefault();
                $('.items-from-ajax-load').fadeIn();
            ...
            $('#sent-documents').on('click', function (e) {
                e.preventDefault();
                $('.items-from-ajax-load').fadeIn();
                $('.data-row').each(function () {
                    var issuerId = $(this).data('entry-issuerid');
                    if (issuerId == 311045022) {
                        $(this).removeClass('d-none');
                    }else {
                ...
                $('.items-from-ajax-load').fadeOut(250);
            }); // filter with sent documents

            $('#received-documents').on('click', function (e) {
                e.preventDefault();
                $('.items-from-ajax-load').fadeIn();
                $('.data-row').each(function () {
                    var issuerId = $(this).data('entry-issuerid');
                ...
                $('.items-from-ajax-load').fadeOut(250);
            }); // filter with received documents
