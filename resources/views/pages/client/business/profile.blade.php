                ...
                <div class="col-md-3">
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-body box-profile">
                            ...
                            @if ($businessClient->archive)
                                @if (auth()->user()->hasPermission('client_restore'))
                                    <form action="{{ route('businessClients.client_restore', ['id' => 'profile']) }}"
                                        method="POST">
                                        @method('DELETE')
                                        @csrf
                                        <input type="hidden" name="sup_id" id="sup_id" value="{{ $businessClient->id }}">
                                        <button type="submit" style="font-size: 13px;"
                                            class="btn btn-danger btn-block"><b>@lang('site.take_back') @lang('site.from')
                                                @lang('site.archive')</b></button>
                                    </form>
                                @endif
                            @else
                                @if (auth()->user()->hasPermission('client_update'))
                                    <a href="{{ route('businessClients.edit', $businessClient->id) }}"
