    ...
    if (documentPage) {
        ...
        $.ajax({
            ...
            success: function (POPaymentDetails) {
                ...
                $('#available-payment-value').text((availableAmout).toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 2 })).parent().removeClass('d-none');
            }
        });
    }
})
...
$("#adddeductionsForm").submit(function (event) {
    ...
    if ($(this).valid()) {
        ...
        let markup = `<tr><th> ${deductions.length}</th>
                        ...
                        <td>${parseFloat(newDeduction.deduction_value).toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td>
        ...
        changeAvailableAmout();
        $('#available-payment-value').text((availableAmout).toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
    }
});
...
function sumOfDeductions() {
    ...
    $('#deduction-total').val(parseFloat(sumOfDeductionValues).toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
...
function changeAvailableAmout() {
    if (editMode)
        availableAmout = ((totalAmount) - (totalPayments + sumOfDeductionValues)) + (oldValuesInEditMode); // available = totalAmount - (allPayment + currentValues) + oldValues
    else
        availableAmout = ((totalAmount) - (totalPayments + sumOfDeductionValues)); // available = totalAmount - (allPayment + currentValues)
}
