...
function resetAllpayments() {
    ...
    $('#available-payment-value').parent().addClass('d-none');
    $('#sum-of-payment-value').addClass('d-none');
...
function changeSumOfPaymentsValues() {
    ...
    sum += deductions.reduce(function (total, deduction) {
        return total + deduction.deduction_value;
    }, 0);

    if (sum != 0)
        $('#sum-of-payment-value').removeClass('d-none').text(parseFloat(sum).toLocaleString('us', { minimumFractionDigits: 2, maximumFractionDigits: 20 }));
    else
        $('#sum-of-payment-value').addClass('d-none');
}
