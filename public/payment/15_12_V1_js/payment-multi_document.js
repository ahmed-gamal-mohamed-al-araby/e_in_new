    ...
    if ($(this).val()) {
        ...
        $.ajax({
            ...
            success: function(documentPaymentDetails) {
                ...
                purchaseOrder= documentPaymentDetails.purchaseOrder;
                ...
                // po[documentIndex - 1].record_id = data.id;
                // po[documentIndex - 1].totalPayments = totalPayments;
                // po[documentIndex - 1].totalAmount = totalAmount;
                // po[documentIndex - 1].sumOfDeductionValues = 0;
...
$('#add-new-document').on('click', function() {
    ...
    const new_document = $(`
                    ...
                    <div class=" col-md-6 my-2">
                        ...
                        <input type="number" name="total_money[]" class="form-control amount_data total_money" required data-document-index = "${documents.length}"
                    ...
                    <div class="col-md-3">
                        <button type="button" style="margin-top: 34px;display: inline-block;" class="btn btn-warning set-available" id=""> ${language['available_payment']}
                        </button>
                    </div>
...
function validateDocuments() {
    let valid = true;
    const __allDocument = $('.document-container1'); // jQuery
    // console.log(__allDocument);
    const allDocument = document.querySelectorAll('.document-container1'); // Native js
...
$('#documents-container').on('click', '.set-available', function() {
    $amount = $('#available-payment-value').text();
    $amount = $amount.replace(/,/g, '');
    // $amount = (Math.round($amount * 100) / 100).toFixed(10);
    $(this).parents(".document-container").find(".amount_data").val($amount);
});
