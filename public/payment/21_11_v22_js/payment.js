...
$('#client_type').on('change', function() {
    ...
    $('#tax_id_number_or_national_id').val('1');
    $('#client_name').val('');
    $('#client_address').val('');
    let businessClient = $("#tax_id_number_or_national_id");
    ...
    if (selectValue == 'b' || selectValue == 'p') {
        ...
        const urlInputType = selectValue;
        const url = `${subFolderURL}/${urlLang}/reports/getALLClientTax`;
        sendAjaxGet('GET', url, businessClient, getTaxIDClient, {
        ...
        // get data(label and name) value in option selected
        let dataLabel = $(this).find('option:selected').data('label');
        //change label text and input names
        labelOrInputParent.find('label').text(dataLabel);
        const urlInputType = selectValue;
        const url = `${subFolderURL}/${urlLang}/reports/getALLClientTax`;
        sendAjaxGet('GET', url, businessClient, getTaxIDClient, {
...
$('#tax_id_number_or_national_id').on("change", function(e) {
...
$('#addline button[type="button"].close').on('click', function() {
    $('#deduction').prop('selectedIndex', 0);
    $('#deduction').select2();
});
...
function changeAvailableAmout() {
    if (editMode)
        availableAmout = fixedTo20((totalAmount) - (totalPayments + sumOfDeductionValues)) + (oldValuesInEditMode); // available = totalAmount - (allPayment + currentValues) + oldValues
    else
        availableAmout = fixedTo20((totalAmount) - (totalPayments + sumOfDeductionValues)); // available = totalAmount - (allPayment + currentValues)
}
...
function getTaxIDClient(response, targetSelector) {
    $('#select_purchase_order :not(:first)').remove();
    console.log(response);
    targetSelector.attr('disabled', false);
    var response = JSON.parse(response);
    targetSelector.empty();
    targetSelector.append(language['select_tax_id']);
    response.forEach(client => {
        if (client.tax_id_number) {
            targetSelector.append(
                `<option value="${client.tax_id_number}">${client.name} (${client.tax_id_number})</option>`            );
        } else {
            targetSelector.append(
                `<option value="${client.national_id}">${client.name} (${client.national_id})</option>`
            );
        }
    });
    $('.loader-container').fadeOut(250);
...
function sendAjaxGet(method, url, target, successFunction, data) {
