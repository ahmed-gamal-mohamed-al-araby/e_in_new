...
function setAllItems(POItemId, documentItemPrices, documentItemQuantities) {
    ...
    // changeQuantity(numberOfAddedItems);
    let markup = `<tr><th> ${numberOfAddedItems + 1}</th>
                ...
                <td class='new-added-quantity'><input id="currentQnt${numberOfAddedItems}" onkeydown="javascript: return event.keyCode == 69 ? false : true"data-item-id="${numberOfAddedItems}" ${(purchaseOrderType == 'budget') ? 'readonly' : ''}  type="number" class="quantity" name="quantity[]" value = "${items[numberOfAddedItems].quantity}" onkeydown="javascript: return event.keyCode == 69 ? false : true"><input type="hidden" id="itemid${numberOfAddedItems}" value="${POItemId}"><span class="badge badge-success py-2 mx-2 d-none" id="available-value${numberOfAddedItems}">00.00</span></td>
...
$('.tableForItems').on('change', '.quantity', function (e) {
    

    itemeditIndex = $(this).data('itemId');
    setDataIntoModal(itemeditIndex);
    changeQuantity2($(this));
...
function changeQuantity(_that) {
    ...
    // itemid${numberOfAddedItems}
    // $('#itemid' + itemeditIndex).val();
    $('#available-value' + itemeditIndex).text(availableQuantities - value).removeClass('d-none');
    items[itemeditIndex].quantity = value;
    // $('.available-valu').text(availableQuantities - value).parent().removeClass('d-none');
}
function changeQuantity2(_that) {
    itemeditIndex = _that.data('itemId');
    let addedItemQuantities = 0,
        availableQuantities = 0;
    ...
    if (documentType == 'I' || documentType == 'D') { // Invoice or Debit
        ...
        if (value > availableQuantities && !that.attr('readonly')) {
            ...
            updatedqnt = value - availableQuantities;
          
            oldqnt=$('#currentQnt'+itemeditIndex).val();
            $('#exampleModalCenter').modal({backdrop: 'static', keyboard: false}, 'show');
            // $('#exampleModalCenter').modal('show');
            
            $('#updatedqnt').val(updatedqnt);
            $('#itemeditIndex').val(itemeditIndex);
            $('#oldqnt').val(oldqnt);
            // $('#itemid' + itemeditIndex).val();
            $('#itemidmodal').val($('#itemid' + itemeditIndex).val());
            // let errorMessage = [...availableQuantitiesOverflowErrorMessage];
            // errorMessage.splice(1, 0, ` (${value}) `); // add item used quantity in documents
            // errorMessage = errorMessage.join(' ');
...
$("#updateQnt").on("click", function() {
    var quantity_difference= $('#updatedqnt').val();
    var item_id_modal= $('#itemidmodal').val();
    var itemeditIndex= $('#itemeditIndex').val();
    
    $.ajax({
        type: 'post',
        url: `${subFolderURL}/${urlLang}/confirmUpdateItemQuantity`,
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        ...
        success: function (response) {
            ...
            if(response == 1){
                ...
                // alert($('#currentQnt'+itemeditIndex).val());
                $('#currentQnt'+itemeditIndex).attr("disabled", true);
            }
            
        },
       
    });
    // $.post('{{ route('confirm_update_item_quantity') }}', {_token:'{{ csrf_token() }}', quantity_difference:quantity_difference,item_id_modal:item_id_modal}, function(data){
    // });

});
$("#closeUpdateQnt").on("click", function() {
    itemeditIndex=$('#itemeditIndex').val();
    $('#currentQnt'+itemeditIndex).val(0);
    $('#available-value'+itemeditIndex).addClass('d-none');
});
