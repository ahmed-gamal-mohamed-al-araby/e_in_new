...
$(window).on('load', function () {
    ...
    $('.quantity').on('keyup change', function () { // items quantity
        ...
        for (let index = 0; index < items.length; index++) { // sum of all quantites saved items in items array in frontend
            if (items[index].item_id == $('#item_select').val())
                addedItemQuantities += fixedTo8(items[index].quantity);
        }
        ...
        if (editDocumentMode) {
            $('.new-added-quantity').each(function (index, element) {
                if ($('#product_code').val() == $(element).prev().prev().text().trim())
                    addedNewItemsQuantityInEditMode += fixedTo8($(element).text());
            });
        }

        if (editDocumentMode)
            availableQuantities = fixedTo8(fixedTo8(POQuantities) - fixedTo8(documentItemQuantities + addedNewItemsQuantityInEditMode)); // get available quantity -> subtract all used quantites from purchase order quantity + old used in item quantity in another documents
        else
            availableQuantities = fixedTo8(fixedTo8(POQuantities) - fixedTo8(documentItemQuantities + addedItemQuantities)); // get available quantity -> subtract all used quantites from purchase order quantity + old used in item quantity in another documents
        ...
        if (modelTitle.data('add-status') == '0') {
            availableQuantities += fixedTo8(items[itemeditIndex].quantity);
        }
    ...
    $('.item_price').on('keyup change', function () { // price of item
        ...
        for (let index = 0; index < items.length; index++) {
            if (items[index].item_id == $('#item_select').val())
                addedItemPrices += fixedTo8(items[index].item_price);
        }
        ...
        if (editDocumentMode) {
            $('.new-added-itemPrice').each(function (index, element) {
                if ($('#product_code').val() == $(element).prev().prev().prev().prev().text().trim())
                    addedNewItemsPriceInEditMode += fixedTo8($(element).text().replace(/,/g, ''));
                // replace(/,/g, '') To parse a string with a comma thousand separator to a number
            });
        }

        if (editDocumentMode)
            availablePrices = fixedTo8(fixedTo8(POPrices) - fixedTo8(documentItemPrices + addedNewItemsPriceInEditMode)); // get available item_price -> subtract all used item_prices from purchase order budget + old used in item budget in another documents
        else
            availablePrices = fixedTo8(fixedTo8(POPrices) - fixedTo8(documentItemPrices - addedItemPrices)); // get available item_price -> subtract all used item_prices from purchase order budget + old used in item budget in another documents

        // check if in edit
        const modelTitle = $('#exampleModalLongTitle');
        modelTitle.data('add-status');
        if (modelTitle.data('add-status') == '0') {
            availablePrices += fixedTo8(items[itemeditIndex].item_price);
        }
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.status == 'success1') {
                ...
                window.location.href = `${subFolderURL}/${urlLang}/documents/index_document_request`;
            }
